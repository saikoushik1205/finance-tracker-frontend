...
export class ExpensesComponent implements OnInit {
  ...
  loadPersons(): void {
    this.loading = true;
    this.apiService.getPersonsBySection("expenses").subscribe({
      next: (response: any) => {
        if (response.success) {
  ...
  updatePerson(): void {
    ...
    this.apiService.updatePerson(this.editingPersonId, personData).subscribe({
      ...
      error: (error) => {
        alert(error.error?.message || "Failed to update vendor");
      },
    });
  }
  ...
  addPerson(): void {
    if (!this.personForm.name.trim()) {
      alert("Please enter a vendor/person name");
      return;
    }
    ...
    this.apiService.createPerson(personData).subscribe({
      ...
      error: (error) => {
        alert(error.error?.message || "Failed to add vendor");
      },
    });
  }
  ...
  updateTransaction(): void {
    ...
    this.apiService
      ...
      .subscribe({
        ...
        error: (error) => {
          alert(error.error?.message || "Failed to update expense");
        },
      });
  }
  ...
  deletePerson(personId: string): void {
    if (!confirm("Delete this vendor and all expenses?")) return;
